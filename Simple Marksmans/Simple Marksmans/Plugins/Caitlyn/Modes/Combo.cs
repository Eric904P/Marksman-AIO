...
namespace Simple_Marksmans.Plugins.Caitlyn.Modes
{
    internal class Combo : Caitlyn
    {
        public static void Execute()
        {
            ...
            {
                var possibleTargets =
                       EntityManager.Heroes.Enemies.Where(x => x.IsValidTarget(R.Range) && x.Distance(Player.Instance) > (IsUnitNetted(x) ? 1300 : Player.Instance.GetAutoAttackRange()) && !EntityManager.Heroes.Enemies.Where(b => b.NetworkId != x.NetworkId).Any(c => new Geometry.Polygon.Rectangle(Player.Instance.Position, x.Position, 90).IsInside(Prediction.Position.PredictUnitPosition(c, 1300))) && x.TotalHealthWithShields() < Player.Instance.GetSpellDamage(x, SpellSlot.R) && !x.HasUndyingBuffA() && !x.HasSpellShield());
