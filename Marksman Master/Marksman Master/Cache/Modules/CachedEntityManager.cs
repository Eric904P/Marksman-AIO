#region Licensing
// ---------------------------------------------------------------------
// <copyright file="CachedEntityManager.cs" company="EloBuddy">
// 
// Marksman Master
...
{
    internal class CachedEntityManager : CacheModuleBase, IDisposable, ICachable
    {
        ...
        private IEnumerable<AIHeroClient> CachedEnemyChampions { get; set; }
        private IEnumerable<AIHeroClient> CachedAllyChampions { get; set; }
        private IEnumerable<AIHeroClient> CachedAllChampions { get; set; }
        private Dictionary<Func<AIHeroClient, bool>, IEnumerable<AIHeroClient>> CachedEnemyChampionsWithAction { get; set; }
        private Dictionary<Func<AIHeroClient, bool>, IEnumerable<AIHeroClient>> CachedAllyChampionsWithAction { get; set; }
        private Dictionary<Func<AIHeroClient, bool>, IEnumerable<AIHeroClient>> CachedAllChampionsWithAction { get; set; }
        ...
        internal IEnumerable<AIHeroClient> GetChampions(CachedEntityType type)
        {
            ...
            switch (type)
            {
                case CachedEntityType.EnemyHero:
                {
                    ...
                    if (LastScan.TryGetValue(type, out lastScan))
                    {
                        ...
                        {
                            return CachedEnemyChampions;
                        }
                        CachedEnemyChampions = EntityManager.Heroes.Enemies;
                ...
                case CachedEntityType.AllyHero:
                {
                    ...
                    if (LastScan.TryGetValue(type, out lastScan))
                    {
                        if (Game.Time*1000 - LastScan[type] < RefreshRate)
                        {
                            return CachedAllyChampions;
                        }
                        CachedAllyChampions = EntityManager.Heroes.Allies;
                        LastScan[type] = Game.Time*1000;
                    }
                    else
                    {
                ...
                case CachedEntityType.AllHeroes:
                {
                    ...
                    if (LastScan.TryGetValue(type, out lastScan))
                    {
                        ...
                        CachedAllChampions = EntityManager.Heroes.AllHeroes;
                        LastScan[type] = Game.Time*1000;
                    }
                    else
                    {
        ...
        internal IEnumerable<AIHeroClient> GetChampions(CachedEntityType type, Func<AIHeroClient, bool> predicate)
        {
            if (type > CachedEntityType.AllyHero)
            {
                throw new Exception($"Invalid type passed. {type} is not supported by AIHeroClient.");
            ...
            switch (type)
            {
                case CachedEntityType.EnemyHero:
                {
                    ...
                    if (LastScan.TryGetValue(type, out lastScan))
                    {
                        if (CachedEnemyChampionsWithAction == null)
                        {
                            CachedEnemyChampionsWithAction =
                                new Dictionary<Func<AIHeroClient, bool>, IEnumerable<AIHeroClient>>();
                        }

                        if (Game.Time*1000 - LastScan[type] < RefreshRate)
                        {
                            IEnumerable<AIHeroClient> output;

                            if (CachedEnemyChampionsWithAction.TryGetValue(predicate, out output))
                            {
                                return CachedEnemyChampionsWithAction[predicate];
                            }
                        }
                        CachedEnemyChampionsWithAction[predicate] = GetChampions(type).Where(predicate);
                        LastScan[type] = Game.Time*1000;
                    }
                    else
                    {
                ...
                case CachedEntityType.AllyHero:
                {
                    ...
                    if (LastScan.TryGetValue(type, out lastScan))
                    {
                        if (CachedAllyChampionsWithAction == null)
                        {
                            CachedAllyChampionsWithAction =
                                new Dictionary<Func<AIHeroClient, bool>, IEnumerable<AIHeroClient>>();
                        }
                        ...
                        if (Game.Time*1000 - LastScan[type] < RefreshRate)
                        {
                            ...
                            if (CachedAllyChampionsWithAction.TryGetValue(predicate, out output))
                            {
                                return CachedAllyChampionsWithAction[predicate];
                            }
                        }
                        CachedAllyChampionsWithAction[predicate] = GetChampions(type).Where(predicate);
                        LastScan[type] = Game.Time*1000;
                    }
                    else
                    {
                ...
                case CachedEntityType.AllHeroes:
                {
                    ...
                    if (LastScan.TryGetValue(type, out lastScan))
                    {
                        ...
                        if (Game.Time*1000 - LastScan[type] < RefreshRate)
                        {
                            ...
                            if (CachedAllChampionsWithAction.TryGetValue(predicate, out output))
                            {
                                return CachedAllChampionsWithAction[predicate];
                            }
                        }
                        CachedAllChampionsWithAction[predicate] = GetChampions(type).Where(predicate);
                        LastScan[type] = Game.Time*1000;
                    }
                    else
                    {
                        if (CachedAllChampionsWithAction == null)
                        {
                            CachedAllChampionsWithAction =
                                new Dictionary<Func<AIHeroClient, bool>, IEnumerable<AIHeroClient>>();
                        }
        ...
        {
            LastScan?.Clear();
            CachedEnemyChampionsWithAction?.Clear();
            CachedAllyChampionsWithAction?.Clear();
            CachedAllChampionsWithAction?.Clear();
